...
# Usage: python src/DUTS/refine-models/train.py --base_model pfanet --aux_model p64
...
parser.add_argument('--base_model', help='Model Name: Choose one of [pfanet]', type=str, required=True)
...
DATA_DIR = os.path.join(WORKING_DIR, 'data', 'DUTS', 'refine-models', args.base_model)
# Log directory
LOG_DIR = os.path.join(WORKING_DIR, 'src', 'DUTS', 'refine-models', 'logs', args.aux_model)
# Checkpoint directory
CHECKPOINT_DIR = os.path.join(WORKING_DIR, 'src', 'DUTS', 'refine-models', 'checkpoints', args.aux_model)
...
logging.info('==========================================================================================')
logging.info('Training %s aux model on DUTS dataset trained on base model %s', args.aux_model, args.base_model)
logging.info('==========================================================================================')
...
if args.aux_model == 'p0':
    for epoch in tqdm(range(1, total_epoch + 1), position = 0, leave=True):
        ...
        train_epoch_mae_rst = 0
        train_epoch_mae_orig = 0
        ...
        for (x, y1, y2) in tqdm(train_dataloader, leave=True, position=0):
            ...
            train_epoch_mae_rst += mae(y1, pred)   
            train_epoch_mae_orig += mae(y2, pred)

        train_epoch_loss /= trainSteps
        train_epoch_iou_rst /= trainSteps
        train_epoch_iou_orig /= trainSteps
        train_epoch_mae_rst /= trainSteps
        train_epoch_mae_orig /= trainSteps
        ...
        logging.info('train_epoch_mae_rst: %f', train_epoch_mae_rst)
        logging.info('train_epoch_mae_orig: %f', train_epoch_mae_orig)
    ...
    for epoch in tqdm(range(1, total_epoch + 1), position = 0, leave=True):
        ...
        train_epoch_iou_orig = 0
        train_epoch_mae_rst_global = 0
        train_epoch_mae_rst_local = 0
        train_epoch_mae_rst = 0
        train_epoch_mae_orig_global = 0
        train_epoch_mae_orig_local = 0
        train_epoch_mae_orig = 0
        ...
        for (x, y1, y2) in tqdm(train_dataloader, leave=True, position=0):
            ...
            train_epoch_mae_rst_global += mae(y1, pred_global) 
            train_epoch_mae_rst_local += mae(y1, pred_local) 
            train_epoch_mae_rst += (0.5 * mae(y1, pred_global) + 0.5 * mae(y1, pred_local)) 
            train_epoch_mae_orig_global += mae(y2, pred_global)
            train_epoch_mae_orig_local += mae(y2, pred_local)
            train_epoch_mae_orig += (0.5 * mae(y2, pred_global) + 0.5 * mae(y2, pred_local)) 
        train_epoch_loss /= trainSteps
        train_epoch_iou_rst /= trainSteps
        train_epoch_iou_orig /= trainSteps
        train_epoch_mae_rst /= trainSteps
