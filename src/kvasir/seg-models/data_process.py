# -----------------------------------------------------------------------------------------------------------------------
# dataprocess.py: This is the code to preprocess kvasir-seg data 

# Usage: python src/kvasir/seg-models/dataprocess.py
...
def default_loader(id, sat_root, mask_root):
    img = cv2.imread(os.path.join(sat_root,'{}.jpg').format(id))
    mask = cv2.imread(os.path.join(mask_root,'{}.jpg').format(id), cv2.IMREAD_GRAYSCALE)
...
def calculate_rst_iou(gt_list, pred_list, patch_size, key):
    ...
    for i in tqdm(range(len(pred_list)), leave=True, position=0):
        ...
        for j in range(len(gt_patches)):
            ...
            for t in range(1, 21):
                ...
                iou = int_uni(gt_patches[j], pred_th)
                temp.append(iou)
        ...
        best_patch_iou = int_uni(gt_list[i], merged)
    ...
    print('[INFO] RST mIoU for patch size {} on {} set = {:.4f}'.format(patch_size, key, np.mean(best_iou_list)))
    return best_iou_list, th_distribution, best_pred_th

# IST iou calculation
def int_uni_numpy(gt_list, pred_list, key=None):
    th_distribution = []
    iou_distribution = []
    best_pred_th = []
    for i in tqdm(range(len(pred_list)), leave=True, position=0):
...
if __name__ == "__main__":
    ...
    for img, mask, orig_img in tqdm(train_data_loader_iter, position=0, leave=True):
        img = img.permute(0,2,3,1).cpu().numpy()[0]
        mask = mask.permute(0,2,3,1).cpu().numpy()[0]
        orig_img = orig_img.cpu().numpy()[0]
        ...
        ax[1].imshow(img)
        masked = np.ma.masked_where(mask[:,:,0] == 0, mask[:,:,0])
        ax[1].imshow(masked, cmap = cmap,interpolation='none', alpha=0.8)
        ax[1].axis('off')
        ax[1].set_title('Ground Truth Mask')
